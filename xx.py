...
def train_epoch(
    ...
    for cur_iter, (inputs, labels, index, meta) in enumerate(train_loader):
        ...
        # For rest of them use one of the labels to use as labels for mixup
        labels = torch.max(labels, dim=1)[1]
        # labels['verb'] = torch.max(labels['verb'], dim=1)[1]
        # labels['noun'] = torch.max(labels['noun'], dim=1)[1]
...
def eval_epoch(val_loader, model, val_meter, cur_epoch, cfg, writer=None):
    ...
    for cur_iter, (inputs, labels, _, meta) in enumerate(val_loader):
        ...
        # For rest of them use one of the labels to use as labels
        # labels = torch.max(labels, dim=1)[1]
        # labels['verb'] = torch.max(labels['verb'], dim=1)[1]
        # labels['noun'] = torch.max(labels['noun'], dim=1)[1]
    ...
    if cfg.WANDB.ENABLE and du.is_master_proc(cfg.NUM_GPUS * cfg.NUM_SHARDS):
        wandb.log(
                    {
                        "Val/top1_err": stats["top1_err"],
                        "Val/top5_err": stats["top5_err"],
                        "Val/min_top1_err": stats["min_top1_err"],
                        "Val/min_top5_err": stats["min_top5_err"],
                        "val_step": len(val_loader) * cur_epoch,
                    },
                )
...
def train(cfg):
    ...
    # For omnivore, you need to use single label
    train_meter = TrainMeter(len(train_loader), cfg)
    val_meter = ValMeter(len(val_loader), cfg)
    ...
    elif cfg.WANDB.ENABLE and du.is_master_proc(
        cfg.NUM_GPUS * cfg.NUM_SHARDS
    ):
        wandb.init("Omnivore", config=cfg)
        wandb.watch(model)
        writer = None
    else:
        writer = None
